info( logger, "TWITTER_PROJECT::testando modelo XGBoost Amostra Estratificada" )

info( logger, "TWITTER_PROJECT::tunning XGBoost" )

iteracoes <- 1 # numero de iteracoes para tunning
...
for(iter in 1:iteracoes){
  ...
  info( logger, paste( "TWITTER_PROJECT:Iteracao Tunning XGBoost Amostra Estratificada", iter ) )
  cat('Iteracao Tunning XGBoost Amostra Estratificada', iter, '\n')
...
#                                dados_tm_tweets, tweets_lsa)
  ...
  # Modelo XGBoost #
  dadosTreino <- xgb.DMatrix(data = as.matrix( dadosTreino[, -1] ), label = dadosTreino$polaridade)
...
Resultados_Tunning_XG_Estrat <- data.frame(
...
#save(Resultados_Tunning_XG_Estrat, file = "src/modeloXGamostraestrat.RData")
...
rm( Accuracy_valid, Accuracy_train, Best_colsample_bytree_xg, Best_eta_xg, 
   ...
   max_delta_step_xg, max_depth_xg, min_child_weight_xg, Resultados_Tunning_XG_Estrat, 
   subsample_xg, iteracoes, s_seeds, cv.nround, cv.nfold, n_word, N_word )

gc()

info( logger, "MASSMEDIAPOC::teste modelo XGBoost Amostra Estratificada finalizado" )